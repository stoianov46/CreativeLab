...
import { getCaseStudies, getPages, getUi } from "@/content/translations";
...
export async function generateMetadata(
  ...
  const page = getPages(locale).caseStudies;
...
export default async function CaseStudiesPage(props: PageProps<"/[locale]/case-studies">) {
  ...
  const page = pages.caseStudies;
  const ui = getUi(locale);
  const entries = getCaseStudies(locale);
  ...
  const graph = jsonLdGraph([
    ...
    breadcrumbSchema([{ name: ui.nav.home, url: homeUrl }, { name: ui.nav.caseStudies, url }]),
  ]);
  ...
  return (
    <>
      ...
      <Breadcrumbs
        items={[{ label: ui.nav.home, href: "/" }, { label: ui.nav.caseStudies, href: "/case-studies" }]}
        locale={locale}
      />
      <Hero
        h1={page.h1}
      ...
      {entries.length ? (
        <LinkCards
          title={page.h1}
          items={entries.map((entry) => ({ title: entry.title, description: entry.summary, href: `/case-studies/${entry.slug}` }))}
        />
      ) : (
